...
var configSchema = exports_external.object({
  ...
  contractAddress: exports_external.string().startsWith("0x"),
...
`;
var userPrompt = `Determine the outcome for: `;
var fetchGeminiOutcome = (runtime2, question) => {
  const apiKeyResp = runtime2.getSecret({ id: "GEMINI_API_KEY" }).result();
  if (!apiKeyResp)
...
` + userPrompt + question }]
  ...
  const req = {
    url: `https://generativelanguage.googleapis.com/v1beta/models/${runtime2.config.geminiModel}:generateContent?key=${apiKey}`,
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body
  };
  let finalResponse = runtime2.runInNodeMode(() => {
    return httpClient.sendRequest(runtime2, req).result();
  }, consensusIdenticalAggregation())();
  while (finalResponse && typeof finalResponse.result === "function") {
    finalResponse = finalResponse.result();
  }
  const response = finalResponse;
  runtime2.log(`Response settled: statusCode=${response?.statusCode}`);
  if (!ok(response)) {
...
var reportOutcome = (runtime2, marketId, geminiResponse) => {
  ...
  if (result === "NO")
    ...
    outcomeUint = 2;
  const reportData = encodeAbiParameters(parseAbiParameters("uint256 marketId, uint8 outcomeUint, uint16 confidenceBps, string evidenceURI"), [marketId, outcomeUint, confidence, "Gemini Search Grounding"]);
...
var eventAbi = parseAbi(["event SettlementRequested(uint256 indexed marketId, string question)"]);
var eventSignature = "SettlementRequested(uint256,string)";
var onSettlementRequested = async (runtime2, log) => {
  ...
  runtime2.log(`Settlement requested for Market ${marketId}: "${question}"`);
  const geminiResult = fetchGeminiOutcome(runtime2, question);
  runtime2.log(`Gemini result: ${JSON.stringify(geminiResult)}`);
  const txHash = reportOutcome(runtime2, marketId, geminiResult);
  runtime2.log(`Report submitted. Tx Hash: ${txHash}`);
  return "SUCCESS";
};
